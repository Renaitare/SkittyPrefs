...
- (void)loadView {
    ...
    if (self.settings[@"tintColor"])
        self.settings[@"tintColor"] = [UIColor colorFromHex:self.settings[@"tintColor"]];
    if (self.settings[@"textColor"])
        self.settings[@"textColor"] = [UIColor colorFromHex:self.settings[@"textColor"]];
    if (self.settings[@"darkTextColor"])
        self.settings[@"darkTextColor"] = [UIColor colorFromHex:self.settings[@"darkTextColor"]];
    if (self.settings[@"headerColor"])
        self.settings[@"headerColor"] = [UIColor colorFromHex:self.settings[@"headerColor"]];
    if (self.settings[@"darkHeaderColor"])
        self.settings[@"darkHeaderColor"] = [UIColor colorFromHex:self.settings[@"darkHeaderColor"]];
    if (self.settings[@"tintColor"])
        self.themeColor = self.settings[@"tintColor"];
}
...
- (void)viewDidLoad {
    ...
    self.iconView.image = [UIImage imageWithContentsOfFile:[[self resourceBundle] pathForResource:@"icon"
                                                                                           ofType:@"png"]];
...
- (void)setNavbarThemed:(BOOL)enabled {
    ...
    if (enabled) {
        ...
        if (self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
            if (self.settings[@"darkHeaderColor"])
                bar.barTintColor = self.settings[@"darkHeaderColor"];
            if (self.settings[@"darkTextColor"])
                bar.tintColor = self.settings[@"darkTextColor"];
        }
        bar.translucent = NO;
...
- (NSMutableArray *)specifiers {
    if (!_specifiers) {
        ...
        for (PSSpecifier *specifier in _specifiers) {
            if ([self.requiredToggles objectForKey:[specifier propertyForKey:@"key"]]) {
                [self.requiredToggles setObject:[self readPreferenceValue:specifier]
